 ...
 *
 * This file is used for generating tabular output where needed, based on the
 * supplied array of table-cell contents.
 *
...
<?php
  for ($row = 1; $row <= sizeof($list_box_contents); $row++) {
    ...
    for ($col = 0; $col < sizeof($list_box_contents[$row]); $col++) {
      ...
      if (isset($list_box_contents[$row][$col]['align']) &&
          $list_box_contents[$row][$col]['align'] != '') {
      ...
      if (isset($list_box_contents[$row][$col]['text'])) {
        echo "<{$cell_type} {$c_params}> {$list_box_contents[$row][$col]['text']}</$cell_type>\n";
      }
    }
  echo "</tr>";
  }
